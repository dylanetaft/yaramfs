...
Bring up NIC(s) for netroot using **MAC id** as the config key (lowercase hex, no colons: `aa:bb:cc:dd:ee:ff` → `aabbccddeeff`). iBFT and static CFG share the same apply path (match iface by MAC → up → optional VLAN → optional static IPv4 and/or IPv6 RA).
##### Phases
  - Prepare
     - No-op
  - Boot
     ...
     - Settle: ping address-iface IPv4 `brd` (from `ip`) or `ff02::1` if no v4 brd; any reply (incl. self) counts; `YARAMFS_CFG_BOOT_NETROOT_SETTLE` wall-clock seconds via `date +%s` (default 30; `0` skips; `sleep 1` between failed probes); die on timeout
